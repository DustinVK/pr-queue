...
Both providers start behind a fixed shell gate. An exec-safe control pipe keeps its close-on-exec write end exclusively in the coordinator. Only the shell receives the read end, and closes it before executing the agent. The coordinator saves nonempty child PID/start/group identity before releasing the gate; coordinator death before release produces EOF without agent execution. Version-2 owner files record this lifecycle. Split-root dry runs use version 3 with the same process identity plus a strictly validated temporary artifact root under the private state directory; persistent ownership remains until that root is removed normally or by recovery, so interrupted removal is retried safely. With the global run lock held, recovery also removes private, empty, direct `run-*` directories left before version-3 ownership was recorded; it retains and reports unsafe or nonempty candidates. Legacy unversioned Claude owners retain session-ID interrupted-start discovery, independent of the currently selected provider.
