...
func TestPublishRepeatedDryRunUsesLastValueForRecovery(t *testing.T) {
	a, _, out := runFixture(t)
	s, err := store.Open(t.Context(), a.paths.Database)
	if err != nil {
		t.Fatal(err)
	...
	if code := a.run(t.Context(), []string{"publish", "owner/repo#1", "--event", "COMMENT", "--dry-run", "--dry-run=false", "--json"}); code != 0 {
		t.Fatalf("live publish: code=%d stdout=%s", code, out.String())
	}
	var status string
	if err := s.DB.QueryRow("SELECT status FROM review_runs WHERE id=?", r.ID).Scan(&status); err != nil {
	...
	if status != "failed" {
		t.Fatalf("last dry-run value was false but recovery left run %q", status)
	}
}
